...
class TFTAdapter:
    ...
    def __init__(self, config: ConfigHelper) -> None:
        ...
        self.filament_sensor: str = config.get('filament_sensor_name')
    ...
    async def _process_klippy_ready(self) -> None:
        ...
        sub_args: Dict[str, Optional[List[str]]] = {
            ...
            "idle_timeout": None,
            "probe": None,
            f"filament_switch_sensor {self.filament_sensor}": None
        }
    ...
    def _send_to_tft(self, message=None, command=None, action=None, error=None) -> None:
        ...
        logging.info("response: %s", formatted_msg)
        byte_resp = (msg + "\n").encode("utf-8")
        self.ser_conn.ser.write(byte_resp)

    async def _monitor_print_status(self) -> None:
        """Monitor the print status and send notifications."""
        while True:
            await asyncio.sleep(1)
    ...
    def _report_software_endstops(self) -> None:
        ...
        filament_sensor=self.printer_state.get(f"filament_switch_sensor {self.filament_sensor}", {})
